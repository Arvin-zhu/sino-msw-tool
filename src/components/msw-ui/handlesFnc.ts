...
export function existInGroup(mock: IGroupDataItem, mocks: Record<string, {data: IGroupDataItem[], isEnable: boolean}>) {
	return !!mocks[mock.group]?.data.find(im => (im.request.url.host + im.request.url.pathname) === (mock.request.url.host + mock.request.url.pathname))
}

export function resetHandlers(groupsRequest: groupsRequestType) {
		//创建handlers,进行动态拦截
		...
		Object.keys(groupsRequest).forEach(_groupKey => {
			if (groupsRequest[_groupKey].isEnable) {
				groupsRequest[_groupKey].data.forEach(request => {
					if (!request.disabled) {
						const handler = rest[request.request.method.toLowerCase() as keyof typeof rest]?.(request.request.url.href, (req, res, ctx) => {
							return res(
		...
		window._msw_worker.resetHandlers(...handlers);
}

export function activeGroupRequest(groupKey: string, groupsRequest: groupsRequestType, active: boolean) {
...
export function judgeHavaGroupHandlers(groupMock: Record<string, {data: IGroupDataItem[], isEnable: boolean}>) {
	let hasData = false;
	if (!groupMock) {
		return hasData;
	}
...
export function importStorageGroupData(data: string | object) {
	if (!data) return;
	try {
		if (data) {
