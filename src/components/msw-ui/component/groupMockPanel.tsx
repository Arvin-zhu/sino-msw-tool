  ...
  return (
    <div className={'msw_group_content'}>
      <div className="msw_group_top">
        ...
        <Upload
          callBack={(data) => store.importGroupData(data)}
          btnText={'导入配置'}
...
const GroupMockPanelItem = observer(
  (props: {
    groupData: Record<string, { data: IGroupDataItem[]; isEnable: boolean }>;
    groupKey: string;
  }) => {
    const { store } = useStores();
      ...
      <div className={'msw_group_content_item'}>
        ...
        <div className="msw_group_content_item_detail">
          <div className="msw_group_content_item_input">
            <ListData
              data={groupData[groupKey]?.data || []}
              operationClick={operationClick}
            />
          </div>
          <CheckboxMsw
            checked={groupData[groupKey].isEnable}
