...
export const AddMockTextAreaComponent = forwardRef((props: {
  ...
  const { request,setTextJson, setShowSwaggerModal, isEdit, saveData, errorMsg, delayRes, setDelayRes, statusCode, onStatusChange, requestAlias, groupName, collectionName, urlInput, setUrlInput, changeGroupName, changeAliasName, changeCollectionName} = props;
  return(
        ...
        <div className="msw_mock_detail_wrap_config">
          <div className="msw_mock_detail_wrap_config_inner">
            ...
            <div className={"msw_group_input_wrap"} style={{ marginTop: 10 }}>
              ...
              <div className="msw_detailConfig_item">
                <Upload
                  callBack={(jsonData) => setTextJson(jsonData)}
                />
              </div>
            </div>
            <div className={"msw_group_input_wrap"} style={{ marginTop: 10, marginLeft: 0 }}>
              <span>导入：</span>
              <div className="msw_detailConfig_item">
                <button className={'msw_config_btn'} onClick={() => setShowSwaggerModal(true)}>从swagger导入</button>
              </div>
            </div>
          </div>
        </div>
