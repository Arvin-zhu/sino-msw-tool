...
export const SelectData = (props: ISelectData) => {
  ...
  return <div className='msw_select'>
    <div className='msw_select_input' onClick={() => !disabled && setVisible(true)}>
      <input disabled={!!disabled} placeholder={'分组名称'} value={value}
    ...
    <div className={`msw_select_dropdown ${visible ? 'show' : ''}`}>
      {
        data?.map((im, index) => {
          return <div className='msw_select_dropdown_item' key={im} onClick={() => {
