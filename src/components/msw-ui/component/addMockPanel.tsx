...
export const AddMockPanel = observer(() => {
  ...
  return <div>
    <div>
      <div className={'msw_addMock_filter'}><span>过滤: </span><input value={filterKeywords} onChange={filterChange} placeholder={'过滤拦截的请求'} /></div>
      <table className='msw_request_table'>
        ...
        <tbody>
        {
          paginationMock?.[pageSize - 1]?.map(im => {
            return <tr key={im.id}>
              <td>
                <span style={{display: 'inline-block', width: 50, marginRight: 10}}>{im.method}</span>
                <span className='msw_table_request'>{im.url.pathname}</span>
              </td>
              <td>
                <div style={{textAlign: 'center'}}><button className='small' onClick={() => {
                  setIsAdd(true);
      ...
      }
      {
        paginationMock.length > 1 && <div className='msw_pagination_wrap'>
              ...
              }} disabled={!(!!paginationMock?.length && (pageSize < paginationMock?.length))} className='small'>下一页</button>
          </div>
      }
      {
        store.currentEditGroupRequest && <div style={{marginTop: 20}}>
              <AddMockTextArea isAdd={isAdd} {...store.currentEditGroupRequest!} />
          </div>
      }
    </div>
  </div>
