...
class HandlerMock {
		...
		Object.keys(this.groupRequest).forEach(im => {
			if (this.groupRequest[im]?.isEnable) {
				this.groupRequest[im].data.forEach(request => {
					handledMap[request.request.method + request.request.url.host + request.request.url.pathname] = true;
				})
			}
		})
		...
		worker.events.on('request:unhandled', (req) => {
			if (filterRequest(req)) {
				if (!existRequest(req, this.handleAllRequest)) {
					...
					const findIndex = this.handleAllRequest.findIndex(im => ((im.method + im.url.host + im.url.pathname)) === (req.method + req.url.host + req.url.pathname));
					if (findIndex !== -1) {
						this.handleAllRequest = this.handleAllRequest.splice(findIndex, 1, req);
					}
				}
			}
		})
	...
	addSimpleMock(data: IGroupDataItem) {
		...
		if (!existInGroup(data, this.groupRequest)) {
			this.groupRequest[data.group].data = [...this.groupRequest[data.group].data, data];
		} else {
	...
	deleteGroupItem(groupKey: string, requestItem: IGroupDataItem) {
		if (this.groupRequest[groupKey]?.data) {
			this.groupRequest[groupKey].data = this.groupRequest[groupKey].data.filter(im  => im !== requestItem);
			resetHandlers(this.groupRequest);
		}
	}
	changeGroupItemStatus(groupKey: string, requestItem: IGroupDataItem, isEnable: boolean) {
		if (this.groupRequest[groupKey]?.data) {
			this.groupRequest[groupKey].data = this.groupRequest[groupKey].data.map(im => {
				if (im === requestItem) {
